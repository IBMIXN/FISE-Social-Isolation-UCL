...
class ActionConciergeFetchWeather(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        print(city)
        r = requests.get(url="http://concierge:8080/current-weather", params={
            "CITY_NAME": city,
...
class ActionConciergeFetchAirQuality(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        city = next(tracker.get_latest_entity_values("GPE"), None)
        print(city)
        r = requests.get(url="http://concierge:8080/air-quality", params={
            "CITY_NAME": city,
        }).json()
...
class ActionConciergeFetchDefinitions(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        print(word)
        r = requests.get(url="http://concierge:8080/dictionary", params={
            "WORD": word,
