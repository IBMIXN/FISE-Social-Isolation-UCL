...
class ActionConciergePlaceCall(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        recipe_search = next(tracker.get_latest_entity_values(
            "recipe_search_term"), None)

        r = requests.get(url="http://concierge:8080/recipe", params={
...
class ActionConciergePlaceCall(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        recipe_ingredient = next(
            tracker.get_latest_entity_values("recipe_ingredient"), None)

        r = requests.get(url="http://concierge:8080/ingredient", params={
...
class ActionConciergePlaceCall(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        r = requests.get(url="http://concierge:8080/random-recipe").json()
...
class ActionConciergePlaceCall(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if not recipe_id:
            ...
            r = requests.get(url="http://concierge:8080/recipe-instructions", params={
                "ID": recipe_id}).json()
