...
class ActionConciergeFetchJoke(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        joke_term = next(
            tracker.get_latest_entity_values("joke_category"), None)
        print(joke_term)
        r = requests.get(url="http://concierge:8080/joke", params={
            "TERM": joke_term}).json()
...
class ActionConciergeFetchNews(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        search_term = next(tracker.get_latest_entity_values(
            "news_search_term"), None)
        print(search_term)
        r = requests.get(url="http://concierge:8080/news", params={
            "QUERY": search_term,
            "LANGUAGE": "en"
        }).json()
...
class ActionConciergeFetchBook(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        search_term = next(
            tracker.get_latest_entity_values("WORK_OF_ART"), None)
        print(search_term)
        r = requests.get(url="http://concierge:8080/book", params={
            "QUERY": search_term,
            "LANGUAGES": "en"
        }).json()
...
class ActionConciergeFetchBookCategory(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        category = next(tracker.get_latest_entity_values(
            "book_category"), None)
        print(category)
        r = requests.get(url="http://concierge:8080/book", params={
            "TOPIC": category,
