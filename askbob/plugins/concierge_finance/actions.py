...
class ActionConciergeFetchStocks(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        print(stock)
        r = requests.get(url="http://concierge:8080/stocks", params={
            "SYMBOL": stock,
        }).json()
...
class ActionConciergeFetchCharity(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        search_term = next(tracker.get_latest_entity_values(
            "charity_search_term"), None)

        r = requests.get(url="http://concierge:8080/charity-search", params={
            "QUERY": search_term,
...
class ActionConciergeFetchCharityCity(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        r = requests.get(url="http://concierge:8080/charity-by-city", params={
            "CITY": city,
